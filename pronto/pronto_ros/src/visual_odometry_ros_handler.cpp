...
VisualOdometryHandlerROS::VisualOdometryHandlerROS(ros::NodeHandle& nh) {
  std::string prefix = "/state_estimator_pronto/fovis/";


  std::cout<<"++++++++++++++++++++++"<<std::endl;
  std::cout<<"VisualOdometry getting........."<<std::endl;
  std::cout<<"++++++++++++++++++++++"<<std::endl;
...
RBISUpdateInterface * VisualOdometryHandlerROS::processMessage(const geometry_msgs::PoseWithCovarianceStamped *msg,
                                                               StateEstimator *state_estimator)
{
  ...
  std::cerr << "RECEIVED POSE MEASUREMENT: " <<
            ...
            eigen_utils::getEulerAnglesDeg(pose_meas_.orientation).transpose() <<
               std::endl;
  return vo_module_->processMessage(&pose_meas_, state_estimator);
}
...
bool VisualOdometryHandlerROS::processMessageInit(const geometry_msgs::PoseWithCovarianceStamped *msg,
                                                  const std::map<std::string, bool> &sensor_initialized,
                                                  const RBIS &default_state,
                                                  const RBIM &default_cov,
                                                  RBIS &init_state,
