...
{
    std::cout << "[pronto_ros/ros_frontend.cpp] is running.............." << '\n';
    bool publish_pose;
    ...
    if(nh_.getParam("/state_estimator_pronto/publish_pose", publish_pose)){
        if(publish_pose){
            ...
            if(!nh_.getParam("/state_estimator_pronto/twist_frame_id", twist_frame_id)){
                ROS_WARN_STREAM("Couldn't get param \"twist_frame_id\". Setting default to: \"" << twist_frame_id <<"\"");
            }
            if(nh_.getParam("/state_estimator_pronto/twist_topic", twist_topic)){
            ...
            // try to
            if(!nh_.getParam("/state_estimator_pronto/publish_tf", publish_tf_)){
                ROS_WARN("Couldn't get param \"publish_tf\". Not publishing TF.");
            }
            if(publish_tf_){
                std::string tf_child_frame_id = "base";
                if(!nh_.getParam("/state_estimator_pronto/tf_child_frame_id", tf_child_frame_id)){
    ...
    if(!nh_.getParam("/state_estimator_pronto/utime_history_span", history_span)){
...
void ROSFrontEnd::initializeState()
{
    ...
    if(!nh_.getParam("/state_estimator_pronto/x0/velocity", init_velocity)){
    ...
    if(!nh_.getParam("/state_estimator_pronto/x0/position", init_position)){
    ...
    if(!nh_.getParam("/state_estimator_pronto/x0/angular_velocity", init_omega)){
    ...
    if(!nh_.getParam("/state_estimator_pronto/x0/rpy", init_orient)){
        ROS_WARN_STREAM("Couldn't get default ang velocity. Setting to zero.");
...
void ROSFrontEnd::initializeCovariance(){
    ...
    if(!nh_.getParam("/state_estimator_pronto/sigma0/Delta_xy", sigma_Delta_xy_init)){
        ROS_WARN_STREAM("Couldn't get param sigma0/Delta_xy. Setting to zero.");
    }


    double sigma_Delta_z_init = 0;
    if(!nh_.getParam("/state_estimator_pronto/sigma0/Delta_z", sigma_Delta_z_init)){
        ROS_WARN_STREAM("Couldn't get param sigma0/Delta_z. Setting to zero.");
    }

    double sigma_chi_xy_init = 0;
    if(!nh_.getParam("/state_estimator_pronto/sigma0/chi_xy", sigma_chi_xy_init)){
    ...
    if(!nh_.getParam("/state_estimator_pronto/sigma0/chi_z", sigma_chi_z_init)){
    ...
    if(!nh_.getParam("/state_estimator_pronto/sigma0/vb", sigma_vb_init)){
        ROS_WARN_STREAM("Couldn't get param sigma0/vb. Setting to zero.");
    }

    double sigma_gyro_bias_init = 0;
    if(!nh_.getParam("/state_estimator_pronto/sigma0/gyro_bias", sigma_gyro_bias_init)){
    ...
    if(!nh_.getParam("/state_estimator_pronto/sigma0/accel_bias", sigma_accelerometer_bias_init)){
