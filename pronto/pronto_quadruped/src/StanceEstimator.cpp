    ...
    force_threshold_(force_threshold)
{
    ...
    std::cout<<"!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
    std::cout<<"stance estimator computing"<<std::endl;
    std::cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
                                ...
                                const Vector3d &omegad)
{
    ...
    for(int leg_id  = 0; leg_id < pronto::quadruped::_LEGS_COUNT; leg_id++) {
        ...
        switch(mode_) {
        case THRESHOLD:
            stance[leg_id] = std::abs(grf_[leg_id](Z)) > force_threshold_ ? true : false;
...
void StanceEstimator::getNormalizedGRF(Eigen::Vector4d &normgrf) {
    ...
    double max = normgrf.maxCoeff();   // .maxCoeff() return the max value of the matrix
    if(max > 200) {
        normgrf /= max;
    } else {
