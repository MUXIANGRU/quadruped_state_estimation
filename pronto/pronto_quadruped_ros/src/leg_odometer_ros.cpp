    ...
    pronto::LegOdometer(feet_jacobians, forward_kinematics), nh_(nh)
{
    ...
    std::string legodo_prefix = "/state_estimator_pronto/legodo/";
    int legodo_mode;
    //MXR::note
    //0 means static sigma 5 var and weight 2
    nh_.getParam(legodo_prefix + "legodo_mode", legodo_mode);


    std::cout<<"速度计算模式::"<<std::endl;
    std::cout<<legodo_mode<<std::endl;
    std::cout<<"+++++++++++++++++"<<std::endl;

    setMode(static_cast<LegOdometer::Mode>(legodo_mode));
    std::cout<<"!!!!!!!!!!!!!!!!!!"<<std::endl;
    std::cout<<"legodometer...."<<std::endl;
    std::cout<<"!!!!!!!!!!!!!!!!!!"<<std::endl;
}
