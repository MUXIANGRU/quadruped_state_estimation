       ...
       cfg_(cfg), fcfg_(fcfg), vo_core_(new FusionCore(fcfg_))
{
  ...
  //MXR::note
  //when I try to use "compressed" image,gazebo is crashed??It seems this problem only occur in simulation???
  //So I try to use "raw",but in complex world the match is too slow
  image_a_ros_sub_.subscribe(it_, image_a_string, 30, image_transport::TransportHints("compressed"));
  image_b_ros_sub_.subscribe(it_, image_b_string, 30, image_transport::TransportHints("compressed"));
  ...
  std::cout<<input_imu_topic<<std::endl;
  std::cout<<"###################"<<std::endl;
  ...
  std::cout<<input_body_pose_topic<<std::endl;
  std::cout<<"###################"<<std::endl;
  ...
  std::cout<<output_body_pose_topic<<std::endl;
  std::cout<<"###################"<<std::endl;
...
void StereoOdom::publishDeltaVO() {
    //MXR::note
    //貌似经过两帧初始化之后，就一直发布里程计信息？？？
    std::cout<<"prepare to 发布视觉里程计信息!!!!!!!!!!!!!!!!!!"<<std::endl;
  ...
  if(!vo_core_->getBodyRelativePose(prev_timestamp,
                                    ...
                                    relative_pose))
  {
      ROS_WARN("Don't publish if the retrieval fails");
    return;
  }
  ...
  ROS_INFO( "start to publish delta_vo_msg_!!!!!!!!!!!!!!!!!!");
                                ...
                                const sensor_msgs::ImageConstPtr& image_b_ros)
{
  ...
  //std::cout<<w<<" "<<h<<std::endl;   1280*720
  ...
  stereo_counter++;
    ROS_WARN("We have received [%d] vo frame", stereo_counter);
}
