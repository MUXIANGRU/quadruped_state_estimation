...
{
    //MXR::just for debug
    //_change_reference_frames is true
//    std::cout<<"&&&&&&&&&&&&&&&&"<<std::endl;
//    std::cout<<"_change_reference_frames:"<<_change_reference_frames<<std::endl;
//    std::cout<<"&&&&&&&&&&&&&&&&"<<std::endl;
...
//      std::cout<<"+++++++++++++++++"<<std::endl;
//      std::cout<<"_cur_frame->getNumKeypoints() == 0"<<std::endl;
...
//  std::cout<<"NOTE!!!!!!!!!!!!!11 width:"<<width<<std::endl;
//  std::cout<<"NOTE!!!!!!!!!!!!!11 height:"<<height<<std::endl;
//  std::cout<<"NOTE!!!!!!!!!!!!!11 _use_adaptive_threshold:"<<_use_adaptive_threshold<<std::endl;

  //_use_adaptive_threshold is false
  ...
  //MXR::note:
  //相邻两帧之间的坐标变换???
//    std::cout<<"estimateInitialRotation"<<std::endl;
//  std::cout<<init_rotation_est.w()<<" "<<init_rotation_est.x()<<" "<<init_rotation_est.y()<<" "<<init_rotation_est.z()<<std::endl;

  tictoc("estimateInitialRotation");
  _p->initial_motion_estimate = _p->ref_to_prev_frame.inverse();
  _p->initial_motion_estimate.rotate(init_rotation_est);

//  std::cout<<"_p->initial_motion_estimate.rotate(init_rotation_est):"<<std::endl;
//  std::cout<<_p->initial_motion_estimate.matrix()<<std::endl;
  ...
  _estimator->estimateMotion(_ref_frame,
    ...
    std::cout<<"_estimator->isMotionEstimateValid(): "<<_estimator->isMotionEstimateValid()<<std::endl;
  if(_estimator->isMotionEstimateValid()) {
      std::cout<<"#######################"<<std::endl;
      std::cout<<"_estimator->isMotionEstimateValid()"<<std::endl;
    ...
    _p->pose = _p->pose * _p->motion_estimate;
      ...
      std::cout<<"#######################"<<std::endl;
      std::cout<<"else if(!changed_reference_frames)"<<std::endl;
    ...
    std::cout<<"note!!!!!!!!!!!!  "<<_estimator->isMotionEstimateValid()<<std::endl;
    if(_estimator->isMotionEstimateValid()) {
  ...
  if(_change_reference_frames)
      std::cout<<"#######################"<<std::endl;
      std::cout<<"if(_change_reference_frames)"<<std::endl;
    dbg("Changing reference frames\n");
    //MXR::test the motion update
    std::cout<<_p->motion_estimate.matrix()<<std::endl;
}
...
VisualOdometry::getDefaultOptions()
{
  ...
  // MotionEstimator
  r["inlier-max-reprojection-error"] = "30";
  r["clique-inlier-threshold"] = "0.1";
  r["min-features-for-estimate"] = "3";
